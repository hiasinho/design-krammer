...
(function() {
  ...
  $.mark = {
    ...
    animateMenu: function(mode) {
      ...
      defaults.nav.one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function() {
        if (defaults.nav.hasClass('bounceOutDown')) {
          defaults.nav.hide();
        }
        return defaults.nav.removeClass('animated bounceInUp bounceOutDown');
      });
      if (mode === 'open') {
        defaults.navToggle.addClass('open');
        defaults.nav.show();
        defaults.nav.addClass('animated bounceInUp');
      } else {
        defaults.navToggle.removeClass('open');
        defaults.nav.addClass('animated bounceOutDown');
      }
    },
    stickyNav: function(options) {
      var $nav, defaults, fixAt, navHeight, parentHeight;
      defaults = {
        selector: ".nav"
      };
      if (typeof options === "string") {
      ...
      options = $.extend(defaults, options);
      $nav = $(options.selector);
      parentHeight = $nav.parent().height();
      navHeight = $nav.height();
      fixAt = parentHeight - navHeight;
  ...
  $(document).ready(function() {
    $(window).on('scroll', function(e) {
      return $.mark.stickyNav();
    });
    return $('.nav-menu').click(function() {
      if ($('nav.nav').is(':visible')) {
        return $.mark.animateMenu('close');
      } else {
        return $.mark.animateMenu('open');
      }
    });
  });
